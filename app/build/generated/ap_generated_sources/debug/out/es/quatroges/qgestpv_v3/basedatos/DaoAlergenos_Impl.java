...
public final class DaoAlergenos_Impl implements DaoAlergenos {
  private final RoomDatabase __db;

  private final EntityInsertAdapter<Alergenos> __insertAdapterOfAlergenos;

  private final EntityDeleteOrUpdateAdapter<Alergenos> __updateAdapterOfAlergenos;

  public DaoAlergenos_Impl(@NonNull final RoomDatabase __db) {
    this.__db = __db;
    this.__insertAdapterOfAlergenos = new EntityInsertAdapter<Alergenos>() {
      @Override
      @NonNull
      protected String createQuery() {
        return "INSERT OR REPLACE INTO `alergenos` (`codigo`,`descripcion`,`md5`) VALUES (?,?,?)";
      }

      @Override
      protected void bind(@NonNull final SQLiteStatement statement, final Alergenos entity) {
    ...
    this.__updateAdapterOfAlergenos = new EntityDeleteOrUpdateAdapter<Alergenos>() {
      @Override
      @NonNull
      protected String createQuery() {
        return "UPDATE OR IGNORE `alergenos` SET `codigo` = ?,`descripcion` = ?,`md5` = ? WHERE `codigo` = ?";
      }
  ...
  @Override
  public long insertRegistro(final Alergenos alergeno) {
    return DBUtil.performBlocking(__db, false, true, (_connection) -> {
      return __insertAdapterOfAlergenos.insertAndReturnId(_connection, alergeno);
    });
  }

  @Override
  public void updateRegistro(final Alergenos alergeno) {
    DBUtil.performBlocking(__db, false, true, (_connection) -> {
      __updateAdapterOfAlergenos.handle(_connection, alergeno);
      return null;
    });
  }
  ...
  public Integer recuperaCount() {
    final String _sql = "select count(*) from Alergenos";
  ...
  public Integer recuperaMin() {
    final String _sql = "select min(codigo) from Alergenos";
    return DBUtil.performBlocking(__db, true, false, (_connection) -> {
      final SQLiteStatement _stmt = _connection.prepare(_sql);
      try {
        final Integer _result;
  ...
  public Integer recuperaMax() {
    final String _sql = "select max(codigo) from Alergenos";
    return DBUtil.performBlocking(__db, true, false, (_connection) -> {
      final SQLiteStatement _stmt = _connection.prepare(_sql);
      try {
        final Integer _result;
  ...
  @Override
  public List<Alergenos> recuperaTodo() {
    final String _sql = "select * from Alergenos";
    return DBUtil.performBlocking(__db, true, false, (_connection) -> {
      final SQLiteStatement _stmt = _connection.prepare(_sql);
      try {
        ...
        final List<Alergenos> _result = new ArrayList<Alergenos>();
        while (_stmt.step()) {
          final Alergenos _item;
          _item = new Alergenos();
  ...
  public List<roomRegistrosCRC> recuperaMD5(final int min, final int max) {
    final String _sql = "select codigo as  idfila,ifnull(md5,'')  as crc from Alergenos where codigo between ? and ?";
    return DBUtil.performBlocking(__db, true, false, (_connection) -> {
      final SQLiteStatement _stmt = _connection.prepare(_sql);
      try {
        int _argIndex = 1;
  ...
  public void borraRegistro(final int idfila) {
    final String _sql = "delete from  Alergenos where codigo = ?";
    DBUtil.performBlocking(__db, false, true, (_connection) -> {
      final SQLiteStatement _stmt = _connection.prepare(_sql);
      try {
        int _argIndex = 1;
  ...
  public void borraTodo() {
    final String _sql = "delete from  Alergenos";
    DBUtil.performBlocking(__db, false, true, (_connection) -> {
      final SQLiteStatement _stmt = _connection.prepare(_sql);
      try {
        _stmt.step();
  ...
  public void borraRango(final int min, final int max) {
    final String _sql = "delete from  Alergenos where codigo between ? and ?";
    DBUtil.performBlocking(__db, false, true, (_connection) -> {
      final SQLiteStatement _stmt = _connection.prepare(_sql);
      try {
        int _argIndex = 1;
