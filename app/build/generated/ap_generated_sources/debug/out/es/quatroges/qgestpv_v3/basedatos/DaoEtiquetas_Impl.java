...
public final class DaoEtiquetas_Impl implements DaoEtiquetas {
  private final RoomDatabase __db;

  private final EntityInsertAdapter<Etiquetas> __insertAdapterOfEtiquetas;

  private final EntityDeleteOrUpdateAdapter<Etiquetas> __updateAdapterOfEtiquetas;

  public DaoEtiquetas_Impl(@NonNull final RoomDatabase __db) {
    this.__db = __db;
    this.__insertAdapterOfEtiquetas = new EntityInsertAdapter<Etiquetas>() {
      @Override
      @NonNull
      protected String createQuery() {
        return "INSERT OR REPLACE INTO `etiquetas` (`codigo`,`descripcion`,`md5`) VALUES (?,?,?)";
      }

      @Override
      protected void bind(@NonNull final SQLiteStatement statement, final Etiquetas entity) {
    ...
    this.__updateAdapterOfEtiquetas = new EntityDeleteOrUpdateAdapter<Etiquetas>() {
      @Override
      @NonNull
      protected String createQuery() {
        return "UPDATE OR IGNORE `etiquetas` SET `codigo` = ?,`descripcion` = ?,`md5` = ? WHERE `codigo` = ?";
      }
      ...
      protected void bind(@NonNull final SQLiteStatement statement, final Etiquetas entity) {
        ...
        statement.bindLong(4, entity.getCodigo());
      }
    };
  }

  @Override
  public long insertRegistro(final Etiquetas etiqueta) {
    return DBUtil.performBlocking(__db, false, true, (_connection) -> {
      return __insertAdapterOfEtiquetas.insertAndReturnId(_connection, etiqueta);
    });
  }

  @Override
  public void updateRegistro(final Etiquetas etiqueta) {
    DBUtil.performBlocking(__db, false, true, (_connection) -> {
      __updateAdapterOfEtiquetas.handle(_connection, etiqueta);
      return null;
    });
  }
  ...
  public Integer recuperaCount() {
    final String _sql = "select count(*) from etiquetas";
  ...
  public Integer recuperaMin() {
    final String _sql = "select min(codigo) from etiquetas";
    return DBUtil.performBlocking(__db, true, false, (_connection) -> {
      final SQLiteStatement _stmt = _connection.prepare(_sql);
      try {
        final Integer _result;
  ...
  public Integer recuperaMax() {
    final String _sql = "select max(codigo) from etiquetas";
    return DBUtil.performBlocking(__db, true, false, (_connection) -> {
      final SQLiteStatement _stmt = _connection.prepare(_sql);
      try {
        final Integer _result;
  ...
  @Override
  public List<Etiquetas> recuperaTodo() {
    final String _sql = "select * from etiquetas";
    return DBUtil.performBlocking(__db, true, false, (_connection) -> {
      final SQLiteStatement _stmt = _connection.prepare(_sql);
      try {
        ...
        final List<Etiquetas> _result = new ArrayList<Etiquetas>();
        while (_stmt.step()) {
          final Etiquetas _item;
          _item = new Etiquetas();
  ...
  public List<roomRegistrosCRC> recuperaMD5(final int min, final int max) {
    final String _sql = "select codigo as  idfila,ifnull(md5,'') as crc from etiquetas where codigo between ? and ?";
    return DBUtil.performBlocking(__db, true, false, (_connection) -> {
      final SQLiteStatement _stmt = _connection.prepare(_sql);
      try {
        int _argIndex = 1;
  ...
  public void borraRegistro(final int idfila) {
    final String _sql = "delete from  etiquetas where codigo = ?";
    DBUtil.performBlocking(__db, false, true, (_connection) -> {
      final SQLiteStatement _stmt = _connection.prepare(_sql);
      try {
        int _argIndex = 1;
  ...
  public void borraTodo() {
    final String _sql = "delete from  etiquetas";
    DBUtil.performBlocking(__db, false, true, (_connection) -> {
      final SQLiteStatement _stmt = _connection.prepare(_sql);
      try {
        _stmt.step();
  ...
  public void borraRango(final int min, final int max) {
    final String _sql = "delete from  etiquetas where codigo  between ? and ?";
    DBUtil.performBlocking(__db, false, true, (_connection) -> {
      final SQLiteStatement _stmt = _connection.prepare(_sql);
      try {
        int _argIndex = 1;
